...
parser.add_argument(
    '--dsroot', default='/n/fs/mm-iga/Varmint/slurm_experiments/')
parser.add_argument('--dsname', default='newdataset')
parser.add_argument('--surrogate', default='tanhsimple')

parser.add_argument(
    '--stepper', choices=['init', 'surrogate'], default='surrogate')
...
def simulate(ref_ctrl, ref_vels, cell, dt, T, predict_fun, radii, optimizer, stepper, friction=1e-7):
    ...
    if stepper == 'init':
        ...
        step = stepper.construct_stepper(
            predict_fun, radii, optimkind=optimizer)
    ...
    while TT[-1] < T:
        ...
        if not success:
            print(
                f'Surrogate time stepping diverged... Aborting and returning what we have.')
            return QQ, PP, TT
...
def main():
    ...
    # Do NOT use the seed from simargs.
