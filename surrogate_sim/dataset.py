...
class Ckpt:
    def __init__(self, ckpt_path):
        ...
        self.QQ, self.PP, self.TT, self.radii, self.cell = load_dynamics_simulation(
            sim_path)
...
def parse_tensors_with_cache(ds_root):
    ...
    if os.path.exists(cache_dir):
        ...
        train_oq, train_op, train_rd, train_nq = assemble_into_inputs(
            QQs, PPs, radii)

        QQs, PPs, TTs, radii = combine_instances(test, verbose=True)
        test_oq, test_op, test_rd, test_nq = assemble_into_inputs(
            QQs, PPs, radii)
