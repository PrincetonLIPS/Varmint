...
def read_dataset(permute=True, use_cache=True):
    ...
    if use_cache:
        if os.path.exists(os.path.join(DATASET_ROOT, 'train_new_q.npy')):
            ...
            train_old_q = np.load(os.path.join(DATASET_ROOT, 'train_old_q.npy'))
            train_old_p = np.load(os.path.join(DATASET_ROOT, 'train_old_p.npy'))
            train_new_q = np.load(os.path.join(DATASET_ROOT, 'train_new_q.npy'))
    ...
    instances = [f for f in os.listdir(DATASET_ROOT) if os.path.isdir(os.path.join(DATASET_ROOT, f)) and 'randomradii' in f]
