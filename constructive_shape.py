...
def get_connectivity_matrix(num_x, num_y, arr2lin, units, global_ctrl):
    ...
    spmat = csr_matrix((np.ones_like(all_rows), (all_rows, all_cols)), shape=(
        n_cp, n_cp), dtype=np.int8)
...
class MaterialGrid(object):
    def __init__(self, instr, ncp):
                ...
                if cell_array[i, j][0] != '0':
                    ...
                    if cell_array[i, j][1] != '0':
                        ...
                        if group.isdigit():
                            ...
                            self.fixed_groups[group].append(
                                (len(self.units)-1, 'left'))
                        else:
                            self.traction_groups[group].append(
                                (len(self.units)-1, 'left'))
        ...
        unflat_indices, spmat = get_connectivity_matrix(
            num_x, num_y, self.arr2lin, self.units, self.ctrls)
        ...
        # Each component has 2, one for x and one for y.
        print(f'Number of degrees of freedom: {2 * n_components}.')

        all_mgrid_indices = []
        for patch_indices in unflat_indices:
            mgrid_indices = np.stack(np.meshgrid(
                patch_indices.flatten(), patch_indices.flatten()), axis=-1)
        ...
        if self.fixed:
            ind_array = \
                sum(self.units[i].get_side_index_array(side, npatches)
                    for (i, side) in self.fixed)
            ...
            for group in self.fixed_groups:
                ...
                group_array = \
                    sum(self.units[i].get_side_index_array(
                ...
                group_all_dists = dijkstra(spmat, directed=False, indices=group_indices,
                                           unweighted=True, min_only=True)
                group_all_dists = np.reshape(
                    group_all_dists, self.ctrls.shape[:-1])
        ...
        self.jac_spy_edges = labels.flatten(
        )[all_mgrid_indices].reshape((-1, 2))
        ...
        jac_sparsity_graph = jac_sparsity_graph[self.nonfixed_labels, :]
        self.jac_sparsity_graph = kron(
            jac_sparsity_graph, np.ones((2, 2)), format='csc')
#    save_npz('VERYHUGEsparsemat.npz', self.jac_sparsity_graph)
        ...
        self.jvpmat, self.reconstruct = pattern_to_reconstruction(
            self.jac_sparsity_graph)
        ...
        for group in self.traction_groups:
            ...
            self.traction_group_labels[group] = \
                sum(self.units[i].get_side_orientation(side, npatches)
                    for (i, side) in sides)

        self.all_orientations = np.zeros((npatches, 4)) + \
            sum(self.traction_group_labels[g]
                for g in self.traction_group_labels)
