...
config = config_dict.ConfigDict({
    ...
    'siren': True,
    'siren_n_layers': 5,
    'siren_n_activations': 256,
    'siren_first_omega_0': 5.,
    'siren_hidden_omega_0': 5.,
    ...
    'max_simp_exponent': 4,
    'simp_exponent_update': 0.002,

    'init_volume_penalty': 0.1,
    'max_volume_penalty': 100,
    'volume_penalty_update': 0.1,
...
def construct_simulation(config):
    ...
    def simulate(mat_params):
        ...
        current_x, _, _, _ = optimize(init_x, increment_dict, tractions_dict, ref_ctrl, mat_params)
...
def main(argv):
    ...
    if config.pixel_opt:
        weight_optimizer = optax.adam(config.pixel_lr)
        opt_state = weight_optimizer.init(init_x)
    else:
    ...
    alpha = config.init_volume_penalty
    ...
    for loop in range(config.max_iters):
        ...
        alpha = min(alpha + config.volume_penalty_update, config.max_volume_penalty)
        ...
        if not config.pixel_opt:
            ...
            updates, opt_state = weight_optimizer.update(penalized_sensitivity, opt_state)
