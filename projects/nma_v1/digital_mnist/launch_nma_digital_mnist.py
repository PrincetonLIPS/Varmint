...
config_flags.DEFINE_config_file('config', 'config/digital_mnist/default.py')
...
def main(argv):
    ...
    init_nn_params = np.zeros((10, config.n_disps)) + 1e-3
    ...
    def loss_fn(all_params, digit):
        ...
        if config.freeze_nn:
            ...
            #mat_inputs = nn_fn_t.apply(nn_params, dummy_displacements)
            mat_inputs = nn_params[digit]
            mat_inputs = np.tanh(mat_inputs) * config.max_disp
    ...
    for i in range(iter_num + 1, 100000):
        ...
        if config.rand_digits:
            target_digits = npr.randint(config.num_digits, size=mpi_size)
        else:
            assert mpi_size == config.num_digits
            target_digits = np.arange(config.num_digits)

        loss, grad_loss = loss_val_and_grad(curr_all_params, target_digits[comm.rank])
        ...
        if i % config.eval_every == 0:
            ...
            if comm.rank == 0:
                ...
                for trial in range(config.num_digits):
                    digit = trial
                    curr_nn_params, curr_radii, curr_color_controls = curr_all_params
                    if config.freeze_nn:
                        mat_inputs = np.ones(config.n_disps) * config.freeze_nn_val
                    else:
                        mat_inputs = curr_nn_params[digit]
                        mat_inputs = np.tanh(mat_inputs) * config.max_disp
                        #mat_inputs = nn_fn_t.apply(curr_nn_params, dummy_displacements)
                    curr_ref_ctrl = radii_to_ctrl_fn(curr_radii)
                    ...
                    vid_path = os.path.join(
                            args.exp_dir,
                            f'sim-{args.exp_name}-optimized-{i}-digit-{digit}.mp4')
                    create_movie_mnist(
