...
from heart_matching_geometry import generate_pore_shapes_geometry, generate_rectangular_radii
from generate_hearts import get_shape_target_generator
...
varmint.prepare_experiment_args(
    None, exp_root='/n/fs/mm-iga/Varmint/projects/nma_v1/heart_matching/experiments',
            source_root='n/fs/mm-iga/Varmint/projects/nma_v1/heart_matching')
...
def main(argv):
    ...
    cell, radii_to_ctrl_fn, n_cells, get_central_pore_points, init_mesh_perturb = \
    ...
    # Here we don't actually use a NN. Just learn a single displacement.
    init_nn_params = jnp.array([0.0])
    ...
    def loss_fn(all_params, cps):
        mat_inputs, (radii, mesh_perturb) = all_params
    ...
    param_labels = ('displacement', ('geometry', 'geometry'))
    optimizer = optax.multi_transform(
            {'displacement': nn_optimizer, 'geometry': geometry_optimizer},
            param_labels)
    ...
    # Generate video
    if comm.rank == 0:
        rprint(f'Visualizing initial reference config.')

        # Saving reference configuration
        _, (curr_radii, curr_mesh_perturb) = curr_all_params
        curr_ref_ctrl = radii_to_ctrl_fn(curr_radii, curr_mesh_perturb)
        ...
        target_image_path = os.path.join(
            args.exp_dir,
            f'sim-{args.exp_name}-optimized-init-inspect-ref-config.png')
        fig.savefig(target_image_path)
        plt.close(fig)
    ...
    for i in range(iter_num + 1, config.max_iter):
        ...
        curr_nn_params = jnp.clip(curr_nn_params, -config.freeze_nn_val, config.freeze_nn_val)
        ...
        if i % config.eval_every == 0:
            ...
            if comm.rank == 0:
                ...
                fig, ax = plt.subplots(config.num_eval, 3)
                fig.set_size_inches(15, 5 * config.num_eval)

                for trial in range(config.num_eval):
                    mat_inputs, (curr_radii, curr_mesh_perturb) = curr_all_params
                    target = shape_generator()
                    ...
                    final_x_local, ctrl_seq = \
                            simulate(mat_inputs, curr_radii, curr_mesh_perturb)
                    our_cps = get_central_pore_points(final_x_local)
                    normalized_our_cps = normalize_pore_shape(our_cps)
                    ...
                    plot_ctrl(ax[trial][2], cell.element, curr_ref_ctrl)
