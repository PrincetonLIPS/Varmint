...
def generate_pore_shapes_geometry(config, material):
    ...
    all_dirichlet_labels = {**dirichlet_labels}
    ...
    init_mesh_perturb = np.zeros_like(total_mesh[1:-1, :])
    def radii_to_ctrl(radii, mesh_perturb=None):
        if mesh_perturb is not None:
            modified_total_mesh = total_mesh.at[1:-1, :].add(mesh_perturb)
        else:
        ...
        new_internal_ctrls = UnitCell2D._gen_cell(internal_corners, radial_ncp,
                                                  init_central_radii, normalized=config.normalized_init)
    ...
    return SingleElementGeometry(
        ...
        traction_labels={},
        comm=MPI.COMM_WORLD,
    ), radii_to_ctrl, n_cells, get_central_pore_points, init_mesh_perturb
