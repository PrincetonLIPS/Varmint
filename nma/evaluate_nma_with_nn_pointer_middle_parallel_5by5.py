...
if __name__ == '__main__':
    ...
    grid_str = "C0000 C0000 C0000 C0000 C0000\n"\
    ...
    optimizer = SparseNewtonIncrementalSolver(cell, potential_energy_fn, max_iter=1000,
                                              step_size=1.0, tol=1e-3, ls_backtrack=0.95, update_every=10, dev_id=dev_id)
    ...
    all_target_disps_x = onp.linspace(11.0, 14.0, 5)
    all_target_disps_y = onp.linspace(11.0, 14.0, 5)

    all_target_disps = onp.stack(onp.meshgrid(all_target_disps_x, all_target_disps_y), axis=-1).reshape((-1, 2))

    # all_target_disps = onp.array([
    #     [11.0, 11.0],
    #     [11.0, 14.0],
    #     [14.0, 11.0],
    #     [14.0, 14.0],
    ...
    for i, target_disps in enumerate(all_target_disps[:, :]):
        delta = target_disps - np.array([12.5, 12.5])
        ...
        create_movie_nma(cell.element, ctrl_seq, vid_path, target_disps, comet_exp=None, p1=p1)
