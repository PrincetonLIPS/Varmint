...
if __name__ == '__main__':
    ...
    grid_str = "S0200 00000 00000 00000 00000 00000 00000\n"\
               "C1000 C0000 C0000 C0000 C0000 C0000 C0000\n"\
               "S0003 00000 00000 00000 00000 00000 00000\n"
    ...
    p1 = np.sum(np.abs(radii_to_ctrl_fn(init_radii) - np.array([35.0, 7.5])), axis=-1) < 1e-14

    test_pts = np.array([
        [30.0, 15.0],
    ])

    test_disps = np.array([30.0, 15.0])
    ...
    for i in range(1, 10000001):
        #target_disps = onp.random.uniform(-1.0, 1.0, 1)
        ...
        print(f'Iteration {i} Loss: {loss} Target: {target_disps[0]} EWA Loss: {ewa_loss} Radii Grad Norm: {np.linalg.norm(grad_loss[1])} Time: {time.time() - iter_time}')
