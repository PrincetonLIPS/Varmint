...
class Cell2D:
    def __init__(self, cell_shape, material, instr):
        ...
        self.init_ctrls = self.init_ctrls.reshape(
            -1, self.cs.num_cp, self.cs.num_cp, 2)
        self.index_arr = self.index_arr.reshape(-1,
                                                self.cs.num_cp, self.cs.num_cp)
    ...
    def generate_circular_radii(self, shape):
        one_arc = 0.6 * \
            np.cos(np.linspace(-np.pi/4, np.pi/4, self.cs.num_cp)[:-1])
        init_radii = np.broadcast_to(
            np.tile(one_arc, 4), (*shape, (self.cs.num_cp-1)*4))
        return init_radii
    ...
    def generate_bertoldi_radii(self, shape, c1, c2, L0=5, phi0=0.5):
        ...
        init_radii = np.broadcast_to(
            np.tile(xs_theta, 4), (*shape, (self.cs.num_cp-1)*4))
        return init_radii
    ...
    def get_dynamics_flatten_unflatten(self):
        ...
        def unflatten(flat_pos, flat_vel, fixed_locs, fixed_vels):
            ...
            unflat_pos = jax.ops.index_update(
                unflat_pos, self.nonfixed_labels, flat_pos)
            unflat_vel = jax.ops.index_update(
                unflat_vel, self.nonfixed_labels, flat_vel)
    ...
    def unflatten_dynamics_sequence(self, QQ, PP, fixed_locs, fixed_vels):
        ...
        unflat_pos, unflat_vel = zip(
            *[unflatten(q, p, f, v) for q, p, f, v in zip(QQ, PP, fixed_locs, fixed_vels)])
    ...
    def get_statics_flatten_unflatten(self):
        ...
        def unflatten(flat_pos, fixed_locs):
            ...
            # without space for fixed locations
            flat_pos = flat_pos.reshape((-1, 2))
