...
def get_lmfunc(
    ...
    def body_fun(state):
        ...
        hit_ftol = (nJp/state.nFx)**2 + 2 * \
            (sqrt_lamb*(nDp/state.nFx))**2 <= ftol

        # Compute gamma via More' (above Eqn. 4.5), should be in [-1,0]
        def gamma_fn(): return -((nJp/state.nFx)**2 + (sqrt_lamb*(nDp/state.nFx))**2)

        # Compute mu via More' Eqn. 4.5.
        def mu_fn(gamma): return np.clip(0.5*gamma /
