...
def inspect_residuals(res_fun, old_q, new_q, p, dt, ref_ctrl, fixed_locs, diagD=1.0):
    resids_before = res_fun(
        old_q * diagD, (old_q, p, dt, (ref_ctrl, fixed_locs)))
    resids_after = res_fun(
        new_q * diagD, (old_q, p, dt, (ref_ctrl, fixed_locs)))

    resid_grad = jax.jacfwd(res_fun)(
        old_q * diagD, (old_q, p, dt, (ref_ctrl, fixed_locs)))
    ...
    print(
        f'Rank of GN Matrix: {np.linalg.matrix_rank(resid_grad.T @ resid_grad)}')
    print(
        f'Condition number of GN Matrix: {np.linalg.cond(resid_grad.T @ resid_grad)}')
