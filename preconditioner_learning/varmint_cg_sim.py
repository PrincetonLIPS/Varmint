...
def main():
    ...
    optim_data['JTJs'] = optim_data['jacs'].transpose(
        0, 2, 1) @ optim_data['jacs']
    ...
    def cgsim(i, prec_name, prec_args=()):
        preconditioner = get_preconditioner(
            prec_name, optim_data['JTJs'][i], prec_args)

        return cg_solver(optim_data['grads'][i], optim_data['xs'][i],
                         optim_data['deltas'][i], optim_data['eps'][i], hv_args=(
                             i,),
    ...
    def sim_on_all(prec_name, start=0, end=N, prec_args=()):
        ...
        for i in range(start, end):
            ...
            norm = npla.norm(
                optim_data['JTJs'][i] @ (x_new - optim_data['xs'][i]) + optim_data['grads'][i])
