...
def _cg_steihaug_jit(grad, x, delta, epsilon, Hv_func, precond, hv_args=()):
  ...
  def main_loop(init_state):
    ...
    ind = jax.lax.cond(converged_cond(final_state), lambda _: 2, lambda _: -1, None)
    ind = jax.lax.cond(out_of_region_cond(final_state), lambda _: 1, lambda _: ind, None)
    ind = jax.lax.cond(negative_curvature_cond(final_state), lambda _: 0, lambda _: ind, None)
