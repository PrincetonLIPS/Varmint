...
def main(argv):
    ...
    def simulate(disps, radii):
        ...
        increment_dict = {
            ...
            '97': np.array([0.0, 0.0]),
            '96': np.array([0.0, 0.0]),
            '1': np.array([-disps[3], 0.0]),
            '2': np.array([0.0, -disps[0]]),
            #'3': np.array([0.0, 0.0]),
    ...
    for i in range(args.load_iter + 1, 10000):
        iter_time = time.time()
        target_angles = onp.random.uniform(-np.pi/4, np.pi/4., size=(mpi_size, 1))
        ...
        if i % 10 == 0:
            ...
            # Generate video
            if comm.rank == 0:
                test_angle = onp.random.uniform(-np.pi/4, np.pi/4., size=(1,))
                test_pts = angle_to_points(test_angle)[0]
