...
def main(argv):
    ...
    input_str = "C0000 C0000 C0000 C0000 C0000 C0000 C0000\n"\
    ...
    p1 = np.sum(np.abs(radii_to_ctrl_fn(init_radii) - np.array([15.0, 17.5])), axis=-1) < 1e-14
    p2 = np.sum(np.abs(radii_to_ctrl_fn(init_radii) - np.array([20.0, 17.5])), axis=-1) < 1e-14
    ps = [p1,p2]
    ...
    for i in range(args.load_iter + 1, 10000):
        iter_time = time.time()
        target_angles = onp.random.uniform(0.0, np.pi/4.0, size=(mpi_size, 1))
        ...
        if i % 10 == 0:
            ...
            # Generate video
            if comm.rank == 0:
                test_angle = onp.random.uniform(0.0, np.pi/4.0, size=(1,))
                ...
                create_static_image_nma(cell.element, g2l(optimized_curr_g_pos, all_fixed_locs[-1], radii_to_ctrl_fn(curr_radii)), image_path, test_pts,ps=ps)
